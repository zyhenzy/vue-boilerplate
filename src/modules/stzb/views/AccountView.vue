  ...
  <div class="account">
    <a-space>
      ...
      <a-select
        ...
        placeholder="选择状态"
      >
        <a-select-option v-for="(text, key) in statusTextMap" :key="key" :value="Number(key)">
          {{ text }}
        </a-select-option>
      </a-select>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'apprentice'">
          <span>
            <a-switch
              v-model:checked="record.apprentice"
              @change="handleApprenticeChange(record, $event)"
            />
          </span>
        </template>
...
const showList = computed(() => {
  ...
  if (minPrice.value !== null) {
    ...
    res = res.filter((item) => {
      ...
      return priceToFilter / 100 >= min // ✅ min 已断言不为 null
    })
  }
  ...
  if (maxPrice.value !== null) {
    ...
    res = res.filter((item) => {
      const priceToFilter = item.intermediaryPrice ?? item.price
      return priceToFilter / 100 <= max // ✅ max 已断言不为 null
    })
  }
  return res
})
...
const aiInsert = async () => {
  const res = await requestAiSearch({
    id: conditionId,
    info:
      '7000议价出，14红徐晃蜀骑 13红核弹 11红撸铁 鬼吕网红 。SS5273\n' +
      '\n' +
